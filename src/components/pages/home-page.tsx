...
function StatCard({ item }: { item: Item }) {
  return (
    <div className='flex items-center gap-4 bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-5'>
      ...
      <div>
        <p className='text-2xl font-bold text-primary leading-none'>{item.title}</p>
...
export default function HomePage() {
  ...
  const { data: blogData, isLoading: blogLoading } = useSWR<PostPaginatedResult>(
    ['/post', { page: 1, limit: 3 }],
  );
    ...
    <div className='overflow-x-hidden'>
            ...
            <h1 className='text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-tight tracking-tight'>
              {hero.title?.includes(hero.subtitle ?? 'Premium Steel') ? (
                <>
                  ...
                  <span className='text-secondary'>{hero.subtitle ?? 'Premium Steel'}</span>
      ...
      <section className='py-16 bg-gray-50'>
        <div className='mx-auto container px-6'>
          <div className='flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10'>
            <div>
              {products.subtitle ? <SectionLabel text={products.subtitle} /> : null}
              <h2 className='text-2xl sm:text-3xl font-bold text-gray-900'>
                {products.title ?? 'Our Products'}
              </h2>
              <p className='mt-2 text-gray-500 max-w-xl text-sm'>{products.body ?? ''}</p>
            </div>
            {products.button ? (
              <Button asChild variant='secondary' className='rounded-full flex-shrink-0'>
                <Link href={products.button.href} className='flex items-center gap-2'>
      ...
      <section className='py-20 bg-white'>
        <div className='mx-auto container px-6'>
          ...
          <div className='text-center max-w-2xl mx-auto mb-12'>
            ...
            <p className='mt-3 text-gray-500 text-sm leading-relaxed'>{whyUs.body ?? ''}</p>
          </div>
      ...
      <section className='py-16 bg-gray-50'>
        <div className='mx-auto container px-6'>
          <div className='flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10'>
            <div>
              {services.subtitle ? <SectionLabel text={services.subtitle} /> : null}
              <h2 className='text-2xl sm:text-3xl font-bold text-gray-900'>
                {services.title ?? 'Our Services'}
              </h2>
              <p className='mt-2 text-gray-500 max-w-xl text-sm'>{services.body ?? ''}</p>
            </div>
            {services.button ? (
              <Button asChild variant='outline' className='rounded-full flex-shrink-0'>
                <Link href={services.button.href} className='flex items-center gap-2'>
      ...
      <section className='py-16 bg-white'>
        <div className='mx-auto container px-6'>
          <div className='flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10'>
            <div>
              {blogSection.subtitle ? <SectionLabel text={blogSection.subtitle} /> : null}
              <h2 className='text-2xl sm:text-3xl font-bold text-gray-900'>
                {blogSection.title ?? 'Recent Updates'}
              </h2>
              <p className='mt-2 text-gray-500 max-w-xl text-sm'>{blogSection.body ?? ''}</p>
            </div>
            {blogSection.button ? (
              <Button asChild variant='outline' className='rounded-full flex-shrink-0'>
                <Link href={blogSection.button.href} className='flex items-center gap-2'>
